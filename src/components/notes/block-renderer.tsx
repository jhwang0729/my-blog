...
export function BlockRenderer({
  ...
  const getPlaceholder = (blockType: BlockType): string => {
    switch (blockType) {
      ...
      case 'numbered_list': return 'Numbered list item'
  ...
  const getBlockIcon = (blockType: BlockType) => {
    ...
    switch (blockType) {
      ...
      case 'heading2': return <Heading2 className={iconClass} />
      case 'heading3': return <Heading3 className={iconClass} />
      ...
      case 'callout': return <AlertCircle className={iconClass} />
  ...
  const renderBlockContent = () => {
    switch (block.block_type) {
      case 'heading1':
        return (
          <h1 
            className={`text-3xl font-bold text-gray-900 dark:text-gray-100 leading-tight ${baseStyles}`}
            {...editableProps}
          >
            {block.content}
          </h1>
      ...
      case 'heading2':
        return (
          <h2 
            className={`text-2xl font-semibold text-gray-900 dark:text-gray-100 leading-tight ${baseStyles}`}
            {...editableProps}
          >
            {block.content}
          </h2>
      ...
      case 'heading3':
        return (
          <h3 
            className={`text-xl font-medium text-gray-900 dark:text-gray-100 leading-tight ${baseStyles}`}
            {...editableProps}
          >
            {block.content}
          </h3>
      ...
      case 'bullet_list':
        return (
          <div className="flex items-start space-x-2">
            ...
            <div 
              className={`flex-1 text-gray-900 dark:text-gray-100 leading-relaxed ${baseStyles}`}
              {...editableProps}
            >
              {block.content}
            </div>
      ...
      case 'numbered_list':
        return (
          <div className="flex items-start space-x-3">
            <span className="text-gray-400 font-medium mt-0.5 flex-shrink-0">1.</span>
            <div 
              className={`flex-1 text-gray-900 dark:text-gray-100 leading-relaxed ${baseStyles}`}
              {...editableProps}
            >
      ...
      case 'quote':
        return (
          <div className="border-l-4 border-gray-300 dark:border-gray-600 pl-4">
            <div 
              className={`text-gray-700 dark:text-gray-300 italic leading-relaxed ${baseStyles}`}
              {...editableProps}
            >
              {block.content}
            </div>
      ...
      case 'code':
        return (
          <div className="bg-gray-100 dark:bg-gray-800 rounded-lg p-4 font-mono">
            <div 
              className={`text-sm text-gray-900 dark:text-gray-100 whitespace-pre-wrap ${baseStyles}`}
              {...editableProps}
            >
              {block.content}
            </div>
        ...
        return (
          <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
            <div className="flex items-start space-x-3">
              ...
              <div 
                className={`flex-1 text-blue-900 dark:text-blue-100 leading-relaxed ${baseStyles}`}
                {...editableProps}
              >
                {block.content}
              </div>
      ...
      case 'link':
        if (block.content) {
          ...
          return (
            <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-4 hover:border-gray-300 dark:hover:border-gray-600 transition-colors">
              <a 
                ...
                className="flex items-center space-x-3 text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 transition-colors"
              >
          ...
          return (
            <div 
              className={`text-gray-900 dark:text-gray-100 leading-relaxed min-h-[1.5rem] ${baseStyles}`}
              {...editableProps}
              ...
              <div className="relative group">
                ...
                {isEditable && (
                  <button
                    ...
                    className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 bg-black bg-opacity-50 text-white px-2 py-1 rounded text-xs hover:bg-opacity-70 transition-all"
                  >
                    Replace
                  </button>
                )}
  ...
  return (
    <motion.div
      className={`group relative py-1 ${
        isFocused ? 'ring-1 ring-blue-200 dark:ring-blue-800 rounded' : ''
      }`}
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
    ...
    >
      ...
      {isEditable && (
        <div className="absolute left-0 top-0 -ml-6 opacity-0 group-hover:opacity-100 transition-opacity">
